...
public class JSFunction {
	...
	public static void alertLocation(String msg, String url, JspWriter out) {
		/*
		Java 클래스에서는 JSP의 내장 객체를 사용할 수 없으므로 반드시
		매개변수로 전달받은 뒤 사용해야 한다.
		여기서는 화면에 문자열을 출력하기 위한 out내장객체를 JspWriter 타입으로
		받은 후 사용하고 있다.
		 */
		try {
			String script = ""
	...
	/*
	앞의 2개의 메서드는 JSP에서 out내장객체를 통해 Javascript함수를 실행한다.
	...
	public static void alertLocation(HttpServletResponse resp, String msg, String url) {
		...
		try {
			...
			String script = ""
						  ...
						  +"    location.href='" + url + "';"
						  +"</script>";
			writer.print(script);//스크립트 문자열을 웹브라우저에 출력한다.
		}
		catch(Exception e) {}
	}
	//JS를 통해 경고창을 띄우고 뒤로 이동한다.
